function [coeffs,fun,F,G,H,W] = orr_sommerfeld(N,R,w)
...
I = eye(N-1);
coeffs{5} = I;
coeffs{4} = i*R*diag(1-x(2:N).^2);
coeffs{3} = -2*D2-i*w*R*I;
coeffs{2} = -i*R*(diag(1-x(2:N).^2)*D2) - 2i*R*I;
coeffs{1} = D4+i*w*R*D2;

fun = @(lam) nlevp_monomials(lam,4);
F = @(lam) (1/R)*(coeffs{1} + lam.*(coeffs{2} + lam.*(coeffs{3}+lam*(coeffs{4} +lam*coeffs{5}))));
G = @(lam) -(D2-lam.^2*I);

[~,w2]=chebpts(N+1,2);

H = @(lam) -D2+abs(lam).^2*I;
W = diag(w2(2:end-1));
end
